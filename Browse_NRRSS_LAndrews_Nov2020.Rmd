...
library(urbnmapr) # for state and county boundaries

# note root/working directory
here()

# pull the Alameda County boundary and California state boundary
alameda_county_boundary <- get_urbn_map("counties", sf = TRUE) %>%
  filter(county_name == "Alameda County") %>% # Alameda County boundary
  st_transform(crs = 4326)
ca_state_boundary <- get_urbn_map("states", sf = TRUE) %>%
  filter(state_name == "California") %>% # California state boundary
  st_transform(crs = 4326)

# save common crs for ease of reference
common_crs <- st_crs(alameda_county_boundary)
```
...
# CA projects represented by state_terr_number 6
...
ca_records_sf <- ca_records %>%
  ...
  st_as_sf(coords = c("lon_full_dm", "lat_full_dm")) %>% # saving as spatial object
  st_set_crs(common_crs) # setting crs to match boundary layers
...
alameda_county_records <- alameda_county_records_sf %>% # grabbing spatial object
  st_drop_geometry() %>% # dropping geomtery
