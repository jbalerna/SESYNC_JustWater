...
ggplot() +
  ...
  scale_color_manual(values = c("darkgrey", "royalblue3")) +
  new_scale_color() + 
  geom_sf(data = study_points, aes(color = obs_type), size = 0.4) +
...
## Run first match analysis without a caliper
...
ggplot() +
  geom_sf(data = ca_boundary) +
  geom_sf(data = restoration_terr, fill = "grey", lwd = 0) +
  geom_sf(data = lines_no_caliper, aes(color = euc_distance_km)) +
  scale_color_gradient2(low = "dodgerblue", mid = "darkblue", high = "red",
                        midpoint = 400) +
  new_scale_color() +
...
## Evaluate calipered match results
