...
site_blk_grp_overlap_area <- st_intersection(x = point1_buff, 
                                         y = blk_grp_ca) %>% 
   mutate(site_blk_grp_intersect_area = st_area(.)) 

# view that
mapview(site_blk_grp_overlap_area)
...
# 10 of the block groups have non-valid geometries; i am cleaning them up simply with a 0-meter buffer
...
site_blk_grp_overlap_area <- st_intersection(x = study_points_buff_t, 
                                         y = blk_grp_ca_t) %>% 
   mutate(site_blk_grp_intersect_area = st_area(.)) 
  ### JMLR: warning message: attribute variables are assumed to be spatially constant throughout all geometries 

# backup
site_blk_grp_overlap_area_orig <- site_blk_grp_overlap_area

# check that
mapview(site_blk_grp_overlap_area[1:500,])
...
*For group*
SESYNC JMLR: Decide: what to do with buffered sites that have some % of area not overlapping census block groups (or not overlapping BGs at all)
