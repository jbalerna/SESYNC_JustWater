...
- aesthetics/recreation/education --> drop if alone
...
incorp_cdp <- places(state = "CA", year = 2020) %>%
  rename(place_fip = PLACEFP,
         ...
         area_land = ALAND,
         area_water = AWATER) %>%
  select(place_fip, geo_id, name, name_w_type, type_code, fips_code, area_land, area_water)

ggplot() +
  geom_sf(data = ca_boundary, fill = "lightgrey") +
  geom_sf(data = ms_simplify(incorp_cdp, keep = 0.05), aes(fill = type_code), lwd = 0) +
  theme_minimal()
```
