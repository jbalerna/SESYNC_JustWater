...
# double check how many cases there were before matching
...
## matched data iteration #1 = match_1to1_nearestneighbor_glm_logit
...
# JMLR: double check whether this is taken from n=6500 or n=492
distances <- matched_1$distances
matched_1_data <- matched_1$X
mean(matched_1)

matchit

# pull out dataframe of match results
match_result <- match.data(matched_1)
...
test <- wilcox.test(pctg_bipoc_buff_area ~ obs_type,
  data = match_result2,
  paired = TRUE) # i'm not sure i've done this pairing correctly!

test <- wilcox.test(rest_site, control_site, paired = TRUE)
test
...
test <- wilcox.test(mhi_wtd_buff_area ~ obs_type,
  data = match_result2,
  paired = TRUE) # i'm not sure i've done this pairing correctly!
...
# The final (?) choice of set up for running Moran's I:
