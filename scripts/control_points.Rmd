...
suppressPackageStartupMessages(library(PairedData))
...
```{r}
# load study points
study_points_orig <- st_read("cleaned_data/study_points_midprocess/study_points.gpkg")
glimpse(study_points_orig)

# backup
study_points <- study_points_orig
...
# HERE!
#study_points <- study_points %>%
#  filter(drainage_density < 3) # this doesn't do anything rn
...
check_corr_vars <- c("slope", "drainage_density", "native_aquatic_index",
  "eflows_class", "mean_actual_discharge_cfs", # double check vars are correct
  "csci_score", "precip", "cwip_score", 
  "nitrate_score", "total_pop_buff_area",
  "mhi_wtd_buff_area", "pctg_bipoc_buff_area")
...
match_vars <- c("obs_id", "obs_type",
  ...
  "total_pop_buff_area", "mhi_wtd_buff_area",
  "pctg_bipoc_buff_area")
...
# for reporting out in the methods, check how many incomplete cases got dropped
study_points %>% filter(obs_type == "restoration_site") %>% nrow() # 268 originally
to_match %>% filter(obs_type == "restoration_site") %>% nrow() # 246 now

# backup
saveRDS(to_match, "intermediate_data/to_match.rds")
...
# match on nearest neighbor using glm
matched_1 <- matchit(obs_type_bin ~ slope + drainage_density +
    native_aquatic_index + precip + csci_score +
    eflows_class + mean_actual_discharge_cfs +
...
# plot the distribution of the propensity scores
plot(matched_1, type = "jitter", interactive = FALSE)

# make qq plots
plot(matched_1, type = "qq", interactive = FALSE)

## calculate the optimal caliper 
# (using recommendation of .2 of the standard deviation of the logit of the propensity score)
distances <- matched_1$distances
  # LA: This still needs to be done
  # I'm still not sure if this calculation should be taken from n=6500 
  # or n=492.

##### REPEAT THE ABOVE 

##### matching iteration #2 
# match on nearest neighbor using a GAM
matched_2 <- matchit(obs_type_bin ~ slope + drainage_density +
    native_aquatic_index + precip + csci_score +
    eflows_class + mean_actual_discharge_cfs +
    any_303d + total_pop_buff_area, 
  data = to_match,
  method = "nearest",
  distance = "gam", # changed
  link = "logit")
...
# plot the distribution of the propensity scores
plot(matched_2, type = "jitter", interactive = FALSE)

# make qq plots
plot(matched_2, type = "qq", interactive = FALSE)

## calculate the optimal caliper 
# (using recommendation of .2 of the standard deviation of the logit of the propensity score)
distances <- matched_2$distances
  # LA: This still needs to be done
  # I'm still not sure if this calculation should be taken from n=6500
  # or n=492.
...
# match on nearest neighbor using random forest
matched_3 <- matchit(obs_type_bin ~ slope + drainage_density +
    native_aquatic_index + precip + csci_score +
    eflows_class + mean_actual_discharge_cfs +
    any_303d + total_pop_buff_area, 
  data = to_match,
  method = "nearest",
  distance = "randomforest", # changed
  link = "logit")
...
# plot the distribution of the propensity scores
plot(matched_3, type = "jitter", interactive = FALSE)

# make qq plots
plot(matched_3, type = "qq", interactive = FALSE)

## calculate the optimal caliper 
# (using recommendation of .2 of the standard deviation of the logit of the propensity score)
distances <- matched_3$distances
  # LA: This still needs to be done
  # I'm still not sure if this calculation should be taken from n=6500
  # or n=492.
...
###### SAVE RESULTS FROM THE BEST PROPENSITY SCORING METHOD

# pull out dataframe of match results
match_result <- match.data(matched_####)
...
###### Pctg BIPOC ######
# make boxplot
pctg_bipoc_boxplot <- ggboxplot(match_result, 
  x = "obs_type", y = "pctg_bipoc_buff_area",
  color = "obs_type", add = "jitter", shape = "obs_type",
  #title = "Title here",
  xlab = "Observation Type",
  ylab = "Percentage BIPOC \n(in 1-km neighborhood buffer around site)"
  )
pctg_bipoc_boxplot

# save that
ggsave("intermediate_outputs/pctg_bipoc_boxplot.png",
  width = 6, height = 6,
  units = "in",
  dpi = 600)
...
# make density plot
ggplot(match_result) +
  geom_density(aes(x = pctg_bipoc_buff_area, fill = obs_type), 
    alpha = .6) +
  geom_rug(aes(x = pctg_bipoc_buff_area, y = 0), 
    position = position_jitter(height = 0)) + 
  labs(x = "Percentage BIPOC \n(in 1-km neighborhood buffer around site)") 
ggsave("intermediate_outputs/pctg_bipoc_density_plot.png", width = 6, height = 6)

# make histogram
ggplot(match_result) +
  geom_histogram(aes(x = pctg_bipoc_buff_area, fill = obs_type), 
    alpha = .6) +
  labs(x = "Percentage BIPOC \n(in 1-km neighborhood buffer around site)") 
ggsave("intermediate_outputs/pctg_bipoc_hist.png", width = 6, height = 6)

## paired samples t-test - assumptions check
# yes, samples are paired
# yes, sample size is large enough (n > 30 samples)
# run: shapiro-wilk normality test?
d <- with(match_result,
  pctg_bipoc_buff_area[obs_type == "restoration_site"] -
    pctg_bipoc_buff_area[obs_type == "control"])
shapiro.test(d)
  # no, they are not normally distributed (this will prob be the case for all of them)

# summary stats by group
match_result %>% group_by(obs_type) %>%
  summarise(
    count = n(),
    median = median(pctg_bipoc_buff_area),
    IQR = IQR(pctg_bipoc_buff_area)
  )   
  # LA/JMLR: Save these results in a table for whichever propensity score 
  # model looks best.

# order dataset for pairs line up
match_result2 <- match_result %>%
  arrange(subclass)

# plot paired data
rest_site <- subset(match_result2, 
  obs_type == "restoration_site", pctg_bipoc_buff_area)
control_site <- subset(match_result2, 
  obs_type == "control", pctg_bipoc_buff_area)
pd <- paired(rest_site, control_site)
plot(pd, type = "profile") + theme_bw()

# save that
ggsave("intermediate_outputs/pctg_bipoc_paired_boxplot.png",
  width = 6, height = 6,
  units = "in",
  dpi = 600)

# run wilcoxon signed rank test
w_test <- wilcox.test(pctg_bipoc_buff_area ~ obs_type,
  data = match_result2,
  paired = TRUE)
w_test

# double check that this method gives the same results; it should
w_test2 <- wilcox.test(rest_site, control_site, paired = TRUE)
w_test2
...
##### REPEAT FOR MHI 
# make boxplot
mhi_boxplot <- ggboxplot(match_result, 
  x = "obs_type", y = "mhi_wtd_buff_area",
  color = "obs_type", add = "jitter", shape = "obs_type",
  #title = "Title here",
  xlab = "Observation Type",
...
# save that
ggsave("intermediate_outputs/mhi_boxplot.png",
  width = 6, height = 6,
  units = "in",
  dpi = 600)

# could make density plot if wanted
ggplot(match_result) +
  geom_density(aes(x = mhi_wtd_buff_area, fill = obs_type), 
    alpha = .6) +
  geom_rug(aes(x = mhi_wtd_buff_area, y = 0), 
    position = position_jitter(height = 0)) + 
  labs(x = "Median Household Income\n(in 1-km neighborhood buffer around site)") 
ggsave("intermediate_outputs/mhi_density_plot.png", width = 6, height = 6)


# make a histogram
ggplot(match_result) +
  geom_histogram(aes(x = mhi_wtd_buff_area, fill = obs_type), 
    alpha = .6) +
 labs(x = "Median Household Income\n(in 1-km neighborhood buffer around site)") 
ggsave("intermediate_outputs/mhi_hist.png", width = 6, height = 6)
...
d <- with(match_result,
  mhi_wtd_buff_area[obs_type == "restoration_site"] -
    mhi_wtd_buff_area[obs_type == "control"])
shapiro.test(d)
...
match_result %>% group_by(obs_type) %>%
  summarise(
    count = n(),
    median = median(mhi_wtd_buff_area),
    IQR = IQR(mhi_wtd_buff_area)
  )
...
# plot paired data
rest_site <- subset(match_result2, 
  obs_type == "restoration_site", mhi_wtd_buff_area)
control_site <- subset(match_result2, 
  obs_type == "control", mhi_wtd_buff_area)
pd <- paired(rest_site, control_site)
plot(pd, type = "profile") + theme_bw()

# save that
ggsave("intermediate_outputs/mhi_paired_boxplot.png",
  width = 6, height = 6,
  units = "in",
  dpi = 600)

# run wilcoxon
w_test_m <- wilcox.test(mhi_wtd_buff_area ~ obs_type,
  data = match_result2,
  paired = TRUE) 
w_test_m

# double check wilcoxon
w_test_m2 <- wilcox.test(rest_site, control_site, paired = TRUE)
w_test_m2
...
# this could be set up more beautifully but alas here it is!

# run conditional logistic regression
match_logregression <- clogit(obs_type_bin ~ mhi_wtd_buff_area + 
    pctg_bipoc_buff_area +
    strata(subclass),
  data = match_result)
summary(match_logregression)
AIC(match_logregression)
...
match_logregression <- clogit(obs_type_bin ~ pctg_bipoc_buff_area  +
    strata(subclass),
  data = match_result)
summary(match_logregression)
AIC(match_logregression)

match_logregression <- clogit(obs_type_bin ~ mhi_wtd_buff_area*pctg_bipoc_buff_area  +
    strata(subclass),
  data = match_result)
summary(match_logregression)
residuals(match_logregression)
AIC(match_logregression)

# choose best option based on AIC and Moran's I, below
```
...
match_result_sf <- study_points %>%
  dplyr::select(obs_id, geom) %>%
  inner_join(match_result, by = "obs_id")

# extract lat and lon for use in regressions
match_result_sf <- match_result_sf %>%
...
```{r}
# Global settings for regions with no neighbors
set.ZeroPolicyOption(TRUE)

# Set up for running Moran's I (there are options here we should play with later)
weights = nb2listw(dnearneigh(match_result_sf, 
  d1 = .1*1000, d2=5*1000), # Discuss in January
  style = "C", # Discuss in January 
  zero.policy = TRUE)

# make sure you have the right match results
match_result_sf$res <- residuals(match_logregression)
AIC(match_logregression)

# Moran's
moran.test(match_result_sf$res, weights, alternative = "two.sided")
  # doesn't pass

## try adding a lat/lon term to the logistic regression
# preprocess
match_result_latlon <- match_result_sf %>%
  st_drop_geometry()
  
match_logregression <- clogit(obs_type_bin ~ mhi_wtd_buff_area*pctg_bipoc_buff_area  + lat + lon +
    strata(subclass),
  data = match_result_latlon)

summary(match_logregression)
match_result_latlon$res <- residuals(match_logregression)
AIC(match_logregression)


# Moran's
moran.test(match_result_latlon$res, weights, alternative = "two.sided")
 
# good news, that passes!

### run again for another model
## try adding a lat/lon term to the logistic regression
# preprocess
match_result_latlon <- match_result_sf %>%
  st_drop_geometry()
...
```{r THIS CHUNK ISN"T WORKING}

distance_options <- c('glm', 'gam', 'rpart', 'randomforest', 'mahalanobis')
method_options <- c('nearest', 'optimal', 'genetic')

combos <- expand_grid(distance = distance_options, method = method_options)

# Explicitly write out formula
full_formula <- as.formula(obs_type_bin ~ slope + drainage_density +
    native_aquatic_index + precip + csci_score +
    any_303d + total_pop_buff_area)

# time this because it takes a while to iterate through everything
tic()
all_matches <- combos %>%
  mutate(match = pmap(., function(distance, method) matchit(full_formula,
    data = to_match, 
    distance = distance, 
    method = method)))
toc()
...
# save that
saveRDS(all_matches, "intermediate_data/all_matches.Rds")

# began at 9:55pm, ended at 10:02pm - took only 7 minutes to run! yay!!!!

# JMLR: warning message appears
# Input `match` is `pmap(...)`.
# Warning: (from Matching) The key tuning parameters for optimization were are all left at their default values.  The 'pop.size' option in particular should probably be increased for optimal results.  For details please see the help page and http://sekhon.berkeley.edu/papers/MatchingJSS.pdf
# (from Matching) The key tuning parameters for optimization were are all left at their default values.  The 'pop.size' option in particular should probably be increased for optimal results.  For details please see the help page and http://sekhon.berkeley.edu/papers/MatchingJSS.pdf
```

```{r making smaller version of all_matches to understand the subsequent functions - can delete later}

distance_options_2 <- c('glm', 'gam')
method_options_2 <- 'nearest'
combos2 <- expand_grid(distance = distance_options_2, method = method_options_2)

all_matches2 <- combos2 %>%
  mutate(match = pmap(., function(distance, method) matchit(full_formula,
    data = to_match, 
    distance = distance, 
    method = method)))

all_matches2_small <- all_matches2$match[2]
all_matches2_small
...
## Process the matches - JMLR: this is the code chunk that I can't figure out!

```{r}

# hard coding for now; JMLR correct this later
i = nrow()

# perhaps to_match has to be ordered with the restored sites first?
to_match <- to_match %>% 
  arrange(desc(obs_type))
glimpse(to_match)
  
process_matches <- function(match_obj, to_match, i) {
  match_index <- rep(NA, nrow(to_match))
  match_index[1:268] <- 1:268 # I think this is the # of treatment sites; we should edit this so it's not hard-coded
    match_index[as.numeric(match_obj[[3]][[i]]$match.matrix)] <- 1:268 # testing
  match_index[as.numeric(all_matches2[[3]][[1]]$match.matrix)] <- 1:268 # testing
  #match_index[as.numeric(match_obj$match.matrix)] <- 1:100 # original
  to_match$match_index <- match_index
  matched_points <- st_sf(to_match, geometry = st_geometry(study_points)) %>%
    filter(!is.na(match_index)) %>%
    mutate(restoration = factor(obs_type_bin, labels = c('no', 'yes')))
  i <- i+1
...
```
Error messages being thrown include: all_matches2_w_details <- all_matches2 %>%
  mutate(matched_points = map(match, process_matches, to_match = to_match))
    matched_lines = map(matched_points, create_matched_lines))

1: Problem with `mutate()` input `matched_points`.
ℹ Unknown or uninitialised column: `match.matrix`.
ℹ Input `matched_points` is `map(match, process_matches, to_match = to_match)`. 
2: Unknown or uninitialised column: `match.matrix`. 


Error: Problem with `mutate()` input `matched_points`.
x replacement has 6842 rows, data has 6500
ℹ Input `matched_points` is `map(match, process_matches, to_match = to_match)`.
